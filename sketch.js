...
function preload() {
  ...
  fontui = loadFont("data/SpaceMono-Regular.ttf");
}
...
function setup() {
  ...
  document.getElementById("insertDate").innerHTML = `Latest update: ${d.getDate()}.${d.getMonth()}.${d.getFullYear()}`;
  frameRate(30);
  colorMode(RGB);
...
function gotData(data) {
  ...
  slider1Label.html(`<b>Wind Speed</b> ${windSpeed}m/s`);
  if (perlinRadius > 150) {
  ...
  slider2Label.html(`<b>Rain</b> ${rain}%`);

  // SETTING GROWTH RANGE FROM HUMIDITY
  vortexR = map(humidity, 0, 100, 75, 275);
  slider4.value(vortexR);
  slider4Label.html(`<b>Humidity</b> ${humidity}%`);

  // SETTING GROWTH FROM CLOUD COVERAGE
  grow = map(clouds, 0, 100, 15, 100);
  slider5.value(grow);
  slider5Label.html(`<b>Cloud Coverage</b> ${clouds}%`);

  // SETTING DESCRIPTION AT THE TOP
  descriptionText.html(
    `<b>Mediona, Alt Penedès.</b> ${description}.<br>Long:${weather.coord.lon} Lat: ${weather.coord.lat}  Temp: ${temp} ℃. Humidity: ${humidity} %.<br>Cloud coverage: ${clouds} %. Wind speed: ${windSpeed} m/s.`
  );
  ...
  //slider6Label.html(`<b>Temperature</b> ${temp} ℃`);
